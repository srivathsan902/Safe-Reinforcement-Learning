...
def main(dir_name, params):
            ...
            if params['main'].get('update', False):
                ...
                if update_from:
                    if os.path.exists(update_from):
                        episode_num = update_from.split('/')[-1]
                        # Create new directory if it doesn't exist
                        new_dir_name = os.path.join(dir_name, episode_num)
                        ...
                        for item in os.listdir(update_from):
                            ...
                            new_item_path = os.path.join(new_dir_name, item)
    ...
    if os.path.exists(os.path.join(dir_name,'episode_safety_calls.npy')):
        episode_safety_calls = np.load(os.path.join(dir_name,'episode_safety_calls.npy')).tolist()
        episode_safety_calls = episode_safety_calls[:start_episode]
    else:
        episode_safety_calls = [0]*start_episode
    ...
    if wandb_enabled:
        ...
        for i in range(len(episode_rewards)):
            ...
            wandb.log({
                ...
                'No. of Corrective Actions': episode_safety_calls[i],
            })

    training_gen = train(env, agent, dir_name, params, start_episode)

    try:
        for episode, reward, cost, percent_safe_actions, safety_calls in training_gen:
            ...
            if wandb_enabled:
                wandb.log({
                    ...
                    'No. of Corrective Actions': safety_calls,
                })
    ...
    np.save(os.path.join(dir_name,'episode_safety_calls.npy'), np.array(episode_safety_calls))
